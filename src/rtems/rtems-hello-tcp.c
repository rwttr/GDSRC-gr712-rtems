...
void test_network (void); // Forward declarations
...
int getrusage(int ignored, struct rusage *ru)
{
	struct timeval now;
...
static void rtems_ttcp_exit (int code)
{
...
char *__progname;
static void ttcpTask (rtems_task_argument arg)
{
	int code;
	int argc;
	char arg0[10];
 ...
 *
 *  ttcp.c,v 1.4 1999/02/10 19:57:04 joel Exp
 */

#ifndef lint
static char RCSid[]  __attribute__((unused)) = "ttcp.c 1.4";
#endif
...
"ttcp: -b option ignored: SO_SNDBUF/SO_RCVBUF socket options not supported\n");
  ...
  {
		sinme.sin_port =  htons(port); /* rcvr */
	}
	...
	if (udp && buflen < 5)
  {
	...
	if (trans)
  {
	   fprintf(stdout, "ttcp-t: buflen=%d, nbuf=%d, align=%d/%d, port=%d", buflen, nbuf, bufalign, bufoffset, port);
 	    if (sockbufsize)
  ...
  {
	    fprintf(stdout, "ttcp-r: buflen=%d, nbuf=%d, align=%d/%d, port=%d",	buflen, nbuf, bufalign, bufoffset, port);
 	    if (sockbufsize)
 		     fprintf(stdout, ", sockbufsize=%d", sockbufsize);
 	    fprintf(stdout, "  %s\n", udp?"udp":"tcp");
	}
...
#define END(x)	{while(*x) x++;}
	...
	for (; *cp; cp++)  {
		...
		case 'U':
			...
			sprintf(outp,"%lld.%01ld", tdiff.tv_sec, tdiff.tv_usec/100000);
			END(outp);
			break;
